...
async function handleSubmit() {
  ...
  try {
    await portfolioStore.addAsset({
      ...
      currentPrice: form.value.currentPrice || form.value.buyPrice
    });
    ...
    error.value = 'Failed to add asset. Please try again.';
  } finally {
    loading.value = false;
  }
}
...
<template>
  <Modal :model-value="modelValue" @update:model-value="emit('update:modelValue', $event)" title="Add New Asset" size="lg">
